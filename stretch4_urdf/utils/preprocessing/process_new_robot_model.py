        ...
        if is_vel_zero or is_eff_zero:
            ...
            if param_key in robot_params:
                ...
                if is_vel_zero and "motion" in p and "max" in p["motion"]:
                    max_vel = p["motion"]["max"].get(
                        "vel", p["motion"]["max"].get("vel_m")
                    )
...
def remove_visual_and_collision_from_sensors_in_base_and_head(urdf_path):
    print(
        "Removing visual and collision tags from sensor frames in base and head links..."
    )
    ...
    for link in root.findall("link"):
        ...
        if any(kw in link_name_lower for kw in sensor_keywords):
            ...
            if is_descendant_of(link_name, target_assemblies) and not is_descendant_of(
                link_name, exclude_assemblies
            ):
                visual = link.find("visual")
...
def finalize_xacro_and_cleanup_meshes(
    stretch_main_xacro, model_name, root_dir, content
):
    import re
    ...
    content = re.sub(
        r'filename="[^"]*meshes/', 'filename="$(arg model_mesh_dir)/', content
    )
...
def generate_xacro_from_base_urdf(model_name, root_dir, xacro_dir):
    ...
    xacro_files = (
        glob.glob(os.path.join(xacro_dir, "*.xacro"))
        if os.path.exists(xacro_dir)
        else []
    )
    ...
    if len(urdf_files) == 0 and len(xacro_files) > 0:
        ...
        if len(xacro_files) == 1:
            ...
            matched = [
                ...
                if os.path.basename(f) in ["stretch_main.xacro", f"{model_name}.xacro"]
            ]
            if matched:
        ...
        print(
            f"No base URDF file found, but found existing xacro: {target_xacro}. Using it directly."
        )
    ...
    if len(urdf_files) != 1:
        print(
            f"Error: Expected exactly one base URDF file in {root_dir}, found {len(urdf_files)}: {urdf_files}"
        )
        return
...
def get_all_model_names():
    ...
    for entry in entries:
        ...
        if (
            ...
            and not entry.endswith("_tools")
            and entry not in ["tools", "utils"]
        ):
            models.append(entry)
            ...
            try:
                ...
                if 0 <= idx < len(all_models):
                    ...
                    if selected in urdf_map:
                        ...
                        print(
                            f"  -> Skipping '{selected}' as a .urdf or .xacro file was not found."
                        )
                else:
