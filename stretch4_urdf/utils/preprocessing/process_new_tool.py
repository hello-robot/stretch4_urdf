...
from stretch4_urdf.utils.preprocessing.update_urdf_with_collision_mesh_filepath import (
    remove_collision_from_optical_links,
    update_urdf_collision_meshes,
)
...
def create_collision_config_if_missing(base_urdf, root_dir):
    ...
    if not os.path.exists(config_path) or is_empty_or_placeholder:
        ...
        with open(config_path, "w") as f:
            yaml.dump(
                {"links": links_dict}, f, default_flow_style=False, sort_keys=False
            )
...
def process_tool_urdf(model_name, root_dir):
    ...
    xacro_files = glob.glob(os.path.join(root_dir, "*.xacro")) + glob.glob(
        os.path.join(root_dir, "xacro", "*.xacro")
    )
    ...
    if len(urdf_files) == 1:
        ...
        print(
            f"No base URDF found, but found existing xacro: {target_file}. Using it directly."
        )
    else:
        print(
            f"Error: Expected exactly one base URDF or XACRO file in {root_dir}, found {len(urdf_files)} URDFs and {len(xacro_files)} XACROs"
        )
        return
    ...
    if root_link != "quick_connect_interface_link":
        print(
            f"Issue: Root link in '{target_file}' is '{root_link}', but it must be named 'quick_connect_interface_link'."
        )
        try:
            ans = (
                input(
                    f"Would you like to add 'quick_connect_interface_link' as the root link with a fixed identity joint to '{root_link}'? (y/N): "
                )
                .strip()
                .lower()
            )
        ...
        if ans == "y":
            ...
            print(
                f"Successfully added 'quick_connect_interface' as the root link to {target_file}."
            )
        else:
            sys.exit(
                f"Error: Root link must be named 'quick_connect_interface', found '{root_link}' in {target_file}"
            )
    ...
    content = re.sub(
        r'filename="[^"]*meshes/', 'filename="$(arg tool_mesh_dir)/', content
    )
    ...
    if os.path.exists(meshes_dir):
        all_meshes = [
            ...
            if f.lower().endswith((".stl", ".dae", ".obj"))
        ]
        unreferenced = [
            os.path.join(meshes_dir, m)
            for m in all_meshes
            if m not in referenced_meshes
        ]
            ...
            for tool_entry in tool_entries:
                ...
                if os.path.isdir(tool_path) and not tool_entry.startswith("__"):
                    ...
                    if len(urdfs) > 0:
                        ...
                        xacros = glob.glob(
                            os.path.join(tool_path, "*.xacro")
                        ) + glob.glob(os.path.join(tool_path, "xacro", "*.xacro"))
                        if len(xacros) > 0:
...
def main(model_names_to_generate: list[str] | None = None):
    ...
    models_to_process = (
        model_names_to_generate if model_names_to_generate else list(urdf_map.keys())
    )
...
if __name__ == "__main__":
            ...
            try:
                ...
                if 0 <= idx < len(all_models):
                    ...
                    if selected in urdf_map:
                        ...
                        print(
                            f"  -> Skipping '{selected}' as a .urdf or .xacro file was not found in the model directory."
                        )
                else:
    ...
    if not model_names_to_generate:
        print(
            "No valid robot model directories selected. Please add a .urdf or .xacro file to the model directory."
        )
        sys.exit(0)
